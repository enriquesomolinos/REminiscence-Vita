...
#ifdef PSVITA
	static const int BTN_LEFT = 7;
	static const int BTN_DOWN = 6;
	static const int BTN_RIGHT = 9;
	static const int BTN_UP = 8;
	static const int BTN_START = 11;
	static const int BTN_SELECT = 10;
	static const int BTN_R1 = 5;
	static const int BTN_L1 = 4;
#endif
...
struct SystemStub_SDL : SystemStub {
	...
	#ifdef PSVITA
		void processEventVita(const SDL_Event &ev, bool &paused);
	#endif
...
void SystemStub_SDL::processEventVita(const SDL_Event &ev, bool &paused) {
	//espace sacar meter pistola
	//mays disparo
	//tab inventario
	//enter usar
	//esc game options
	switch (ev.jbutton.button) {
		case BTN_SELECT:
		{
			if(ev.jbutton.state == SDL_PRESSED){
				...
				int windowH = 0;
				if (_fullscreen) {		
					windowW = _screenW * _scalers[_scaler].factor;
					windowH = _screenH * _scalers[_scaler].factor;
					_fullscreen =false;
					_midscreen = false;
				}else{
					if(_midscreen){
						...
						windowW = _screenW * _scalers[_scaler].factor+100;
						windowH = _screenH * _scalers[_scaler].factor;
						_fullscreen =false;
						_midscreen = true;						
					}
